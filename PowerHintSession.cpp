...
void PowerHintSessionImpl::resetBoost() {
    if (mHandle > 0) {
        ...
        if (mDebug) LOG(DEBUG) << "Handle " << mHandle << " released";
    }
    mHandle = -1;
...
bool PowerHintSessionImpl::taskLoadBoost(int loadType) {
    ...
    if (loadType == LOAD_RESET) {
        if (mTLBHandle > 0) {
            ...
            if (mDebug) LOG(DEBUG) << "Handle " << mTLBHandle << " released";
        }
        mTLBHandle = -1;
        mTLBoostSum = 0;
        return true;
    ...
    if (boostSum != 0) {
        ...
        if (mDebug)
            LOG(DEBUG) << "Handle " << handle << " for threads "
...
void PowerHintSessionImpl::releaseLowCpuUtil() {
    for (int i = 0; i < mNumPowerEfficiencyThreads; i++) {
        ...
        if (mDebug) LOG(DEBUG) << "Released low cpu util hint for thread " << mThreadIds[i];
    }
    mNumPowerEfficiencyThreads = 0;
}
...
void PowerHintSessionImpl::hintLowCpuUtil() {
    ...
    for (auto it = mThreadIds.begin(); it != mThreadIds.end() && count < MAX_THREADS; ++it) {
        ...
        if (mDebug) LOG(DEBUG) << "Set thread " << *it << " to prefer power efficiency";
        ++count;
    }
...
double PowerHintSessionImpl::nextSupportedFPS(double fps) {
    ...
    if (mDebug) LOG(DEBUG) << "Supported FPS: " << *it << " for requested FPS: " << fps;
    return *it;
}
